...
class GameLogic {
    ...
    if (GameValues.currentHandIndex < GameValues.player.hands.length) {
      if (GameValues.player.hands[GameValues.currentHandIndex].isSplitted) {
        SettingsGlobalValues.logger
            .i("Splitted hand ${GameValues.currentHandIndex}, hitting");
        hit(GameValues.player.hands[GameValues.currentHandIndex]);
      } else if (isBlackjack()) {
        SettingsGlobalValues.logger
            .i("BlackJack for hand ${GameValues.currentHandIndex}");
        return await nextHandOrEnd();
      }
    } else {
      SettingsGlobalValues.logger.i("Hand not in index, playing for dealer");
  ...
  static hit(Hand hand) async {
    ...
    if (handValue > 21) {
      SettingsGlobalValues.logger
          .i("Bust at $handValue for hand ${GameValues.currentHandIndex}");
      return await nextHandOrEnd();
    }
    ...
    if (handValue == 21) {
      SettingsGlobalValues.logger
          .i("21 for hand ${GameValues.currentHandIndex}");
      return await nextHandOrEnd();
    }
    SettingsGlobalValues.logger
        .i("$handValue for hand ${GameValues.currentHandIndex}");
    return;
  }
  ...
  static stand(Hand hand) async {
    if (GameValues.currentHandIndex == -1) {
      ...
      SettingsGlobalValues.logger
          .i("${hand.getValue()} for hand ${GameValues.currentHandIndex}");
      return await nextHandOrEnd();
    }
  }
